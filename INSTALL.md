...
## Как это подключается (две разные части)

В kit'е два механизма, и работают они по-разному:

| Часть | Что это | Как срабатывает |
|---|---|---|
| `scripts/*.sh` | **хуки** (код) | запускает сам Claude Code на событиях `UserPromptSubmit` / `Stop` — по записи в `settings.json` |
| `rules/agent-session-ritual.md` | **правило** (текст) | **автозагружается в контекст** из `~/.claude/rules/*.md` при старте сессии — Claude читает и выполняет как инструкцию |

Важно:
- Правило **не «запускается»** — это не процесс. Это markdown, который Claude Code сам подтягивает в контекст из `~/.claude/rules/`. Отдельный `~/.claude/CLAUDE.md` для этого **не нужен**.
- Поэтому достаточно положить файл в `~/.claude/rules/` (шаг 1) и перезапустить Claude Code.
- Правило исполняется «по доброй воле» агента (это инструкция, а не гарантированный код). Хук `Stop` страхует: он журналирует сессию в очередь в любом случае, даже если карточку забыли завести.
...
# правило подхватилось? в новой сессии спроси Claude: "какие правила из ~/.claude/rules загружены?"
# в ответе должен быть agent-session-ritual.md
```
